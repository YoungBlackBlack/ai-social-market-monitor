...
生成时间：2026-06-21T14:42:51.707Z

## 总结

- 产品主表：42 个；HelloTalk/Tandem 相邻唯一覆盖：20 个。
- 近两年融资/新闻：25 条主时间线、37 行合并复核、34 个去重唯一事件。
- IRL/线下真人证据地图：25 行。
- Monitor：9 条 lane、13 条近窗信号、0 条新增提醒；待复核为 28。
...
- 证据：60 组 Exa 查询、625 条结果；类别审计覆盖 6 类，市场判断映射含 14 条 research/financial 证据。
...
- 状态：proved-for-current-evidence
- 证据：精确窗口 2024-06-02 - 2026-06-02；主时间线 25 条，补充候选 12 条，合并复核 37 行，去重唯一事件 34 个，窗口外事件 0 条。
...
- 状态：proved-for-current-evidence-with-monitor-boundary
- 证据：IRL/线下真人覆盖 25 个唯一条目，主表 IRL 样本 14 个；线下真人证据地图 25 行，其中融资/新闻链接 17 行、monitor 链接 5 行、强证据 1 行。
...
- 证据：当前 Exa Websets Monitor 权限不可用，使用 Search API + Codex automation fallback；最近 monitor 9 条 lane、13 条近窗高相关信号、0 条新增提醒；latest alert 待复核 28，recent signal 待复核 0。
...
- 证据：当前 strictCompletionVerdict.status = not-mathematically-complete，presentationReady = false；阻塞型候选 0，monitor 观察型缺口 22，背景/低相关留档 63。
