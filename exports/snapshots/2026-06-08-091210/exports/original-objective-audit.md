...
生成时间：2026-06-08T09:12:06.279Z

## 总结

- 产品主表：42 个；HelloTalk/Tandem 相邻唯一覆盖：19 个。
- 近两年融资/新闻：25 条主时间线、38 行合并复核、35 个去重唯一事件。
- IRL/线下真人证据地图：29 行。
- Monitor：9 条 lane、33 条近窗信号、0 条新增提醒；待复核为 0。
...
- 证据：42 个产品进入主矩阵；HelloTalk/Tandem 相邻语言条目 19 个唯一覆盖，其中 16 个在主表语言产品。
...
- 状态：proved-for-current-evidence-with-monitor-boundary
- 证据：IRL/线下真人覆盖 29 个唯一条目，主表 IRL 样本 14 个；线下真人证据地图 29 行，其中融资/新闻链接 22 行、monitor 链接 14 行、强证据 9 行。
...
- 证据：当前 Exa Websets Monitor 权限不可用，使用 Search API + Codex automation fallback；最近 monitor 9 条 lane、33 条近窗高相关信号、0 条新增提醒；latest alert 待复核 0，recent signal 待复核 0。
...
- 证据：当前 strictCompletionVerdict.status = not-mathematically-complete，presentationReady = true；阻塞型候选 0，monitor 观察型缺口 29，背景/低相关留档 60。
