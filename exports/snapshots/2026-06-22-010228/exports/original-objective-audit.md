...
- IRL/线下真人证据地图：24 行。
- Monitor：9 条 lane、15 条近窗信号、2 条新增提醒；待复核为 0。
...
- 状态：proved-for-current-evidence-with-monitor-boundary
- 证据：IRL/线下真人覆盖 24 个唯一条目，主表 IRL 样本 14 个；线下真人证据地图 24 行，其中融资/新闻链接 17 行、monitor 链接 4 行、强证据 1 行。
...
- 证据：当前 Exa Websets Monitor 权限不可用，使用 Search API + Codex automation fallback；最近 monitor 9 条 lane、15 条近窗高相关信号、2 条新增提醒；latest alert 待复核 0，recent signal 待复核 0。
...
- 证据：当前 strictCompletionVerdict.status = not-mathematically-complete，presentationReady = true；阻塞型候选 0，monitor 观察型缺口 22，背景/低相关留档 63。
