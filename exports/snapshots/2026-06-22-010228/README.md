...
- `exports/latest-snapshot.md` / `exports/latest-snapshot.json` - fixed pointer to the newest timestamped snapshot and manifest.
- `exports/exa-category-coverage-guide.md` - presentation guide explaining how Exa categories map to product, company, funding, people, financial, and research judgments.
...
This repo can run on Railway without keeping a browser runtime in production.

### Recommended shape

- One Railway service running `npm start`
- One mounted volume, for example at `/data`
- Environment variables set in Railway instead of committing `.env`

The runtime server will:

- serve the latest `exports/exa-ai-social-report.html` at `/`
- run the monitor workflow on a schedule inside the same service
- send Feishu webhook notifications when `meta.shouldSend` is true
- seed `data/` and `exports/` into the mounted state directory on first boot, then symlink the live workspace paths to that persistent state
...
STATE_DIR=/data/ai-social-market-monitor
MONITOR_CRON=0 9 * * *
FULL_REBUILD_CRON=30 9 * * 1
SCHEDULE_TIMEZONE=Asia/Shanghai
ENABLE_SCHEDULER=true
RUN_MONITOR_ON_STARTUP=true
ENABLE_FULL_REBUILD=true
ENABLE_BROWSER_VALIDATION=false
```

### Why `ENABLE_BROWSER_VALIDATION=false`

`scripts/validate-deliverable.mjs` uses Playwright + Chromium for real browser QA. That is useful locally, but not required for the hosted Railway path. With `ENABLE_BROWSER_VALIDATION=false`, the scheduled workflow skips the browser validator so Railway does not need Playwright installed.

### Runtime endpoints

- `/` - latest standalone HTML report
- `/healthz` - health probe
- `/statusz` - runtime scheduler/status payload
